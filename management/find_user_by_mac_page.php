<?php
	...
	$query_text = $global_functions->correct_mac_address( $findmac );
	if( !$query_text ) {
		$out_result = $nm_web_interface->create_error_message( "HATALI GÝRÝÞ" , $findmac . "-> Hatalý MAC adres. Lütfen, düzeltip tekrar giriniz !!!" );
		print $out_result;
		die();
	}
	...
	$result_array = $DbUtil->query_eduroam_log_mac( $findmac );
	if( count( $result_array ) == 0 ) {
		$DbUtil->setDatabase( "dhcp" );
		$query_result_dhcp_lease = $DbUtil->query_dhcp_lease_log_mac( $findmac );
		$query_result_dhcp_host = $DbUtil->query_dhcp_host_log_mac( $findmac );
	...
	$query_result_switch = $DbUtil->query_switch_connected_port_mac( $findmac );
